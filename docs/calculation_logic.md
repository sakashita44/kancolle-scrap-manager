...
1. **OR条件(包含関係)**: カテゴリ指定とItem指定が混在する場合,Itemはカテゴリに包含される
...
```javascript
const selectedMissionIds = ['m_ms_daily_scrap_1', 'm_ms_weekly_scrap_1'];
```
...
1. カテゴリ要求数からItem合計数を差し引く
1. 差分が0以下の場合,カテゴリ要求は削除(Itemだけで満たされるため)
...
1. **ロギング**: 開発モードでは各フェーズの中間結果をコンソールに出力し,デバッグを容易にする
...
1. 選択任務1件
1. 選択任務8件(上限)
1. 同じ装備を要求する複数任務(MAX集計)
1. カテゴリとItemの包含関係(差し引き)
1. カテゴリ要求のみ
1. Item要求のみ
1. 存在しない装備IDを参照
1. 同一任務内の重複要求(エラー検出)
1. 複数カテゴリと複数Itemの複雑な組み合わせ

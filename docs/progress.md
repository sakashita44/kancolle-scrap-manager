...
最終更新: 2025-11-28 (v1.0.0-betaリリース準備)
...
* ✅ リリース前動作確認メモ作成 (2025-11-28) (#47 完了)
    * docs/qa_checklist.md新規作成
    * 主要機能の動作確認ポイント記載（基本動作、ブラウザ確認、デプロイ前最終確認）
    * 個人開発に適したシンプルなメモ形式（詳細なチェックリストではなく）
* ✅ README.md整備 (2025-11-28) (#49 完了)
    * ユーザー向け使い方ガイドを中心に再構成
    * AI利用の明記、免責事項を先頭に配置
    * 特徴、基本操作、フィルタ機能、装備・任務管理の説明追加
    * 計算ロジックの例、FAQ、トラブルシューティング追加
    * 文字数制限を実装に合わせて修正（装備名40、任務名50、カテゴリ20）
    * 開発者向け情報は技術情報セクションに集約
* ❌ LocalStorage容量警告 (#45 not planned)
    * 保存失敗時にエラー表示で十分と判断
    * 事前警告機能は個人開発では過剰
    * issue #45をnot plannedでclose
* ✅ データフェッチ廃止とバンドルデータへの統一 (2025-11-28) (#65 完了)
    * **データ配置変更**
        * public/data/*.json → src/data/*.json へ移動（git mv使用）
        * マスタデータをViteビルド時にバンドルする構成に変更
    * **Hooksのリファクタリング**
        * useCategories.js: JSONファイル直接インポートに変更
        * useEquipments.js: JSONファイル直接インポートに変更
        * useMissions.js: JSONファイル直接インポートに変更
        * appVersionパラメータ削除、isMasterフラグはuseMemo内で付与
    * **不要ファイルの削除**
        * src/hooks/useMasterData.js削除（フェッチロジック不要）
        * src/hooks/useFetchWarning.js削除（フェッチ失敗警告不要）
        * src/utils/dataFetch.js削除（フェッチ処理不要）
    * **App.jsx修正**
        * loading/error/dataSourceの返り値削除
        * useFetchWarningの呼び出し削除
        * マスタフェッチ失敗警告バナー削除
        * ローディング表示削除（即座に起動）
    * **ドキュメント更新**
        * docs/design.md: データフロー図をバンドル方式に更新
        * docs/error_handling.md: ネットワークエラーセクション削除
        * CLAUDE.md: データフロー戦略をバンドル方式に更新
    * **効果**
        * 起動時のネットワーク待機時間がゼロ
        * アプリとマスタデータのバージョン不整合を完全に防止
        * フェッチエラー、タイムアウト、フォールバック処理が不要
* ✅ 本番環境デプロイフロー整備 (2025-11-28) (#48 完了)
    * **FTPデプロイスクリプト実装**
        * scripts/deploy.js作成（mainブランチチェック、ビルド、FTPアップロード）
        * ftp-deploy, dotenvパッケージ追加
        * package.jsonにdeployコマンド追加（npm run deploy）
    * **設定ファイル整備**
        * .env.example完全リライト（FTPデプロイ設定のみに簡素化）
        * .gitignoreに.env除外設定あることを確認（既存設定で対応済み）
    * **デプロイフロー**
        * mainブランチチェック（他のブランチからは実行不可）
        * npm run build実行
        * dist/をFTPサーバーへアップロード
    * **セキュリティ**
        * FTP認証情報は.envで管理（リポジトリ外）
        * .env.exampleは汎用的なテンプレート（特定サービス名を含まない）
* ✅ v1.0.0-betaリリース準備 (2025-11-28)
    * **バージョン更新**
        * package.jsonを1.0.0-betaに更新
    * **ベータ版警告の追加**
        * AboutModalにベータ版注意書きセクション追加（マスタデータにダミーデータが含まれる旨を明記）
        * GlobalWarningBannerでベータ版警告バナー追加（type="info"、×ボタンで閉じれる仕様）
    * **favicon追加**
        * public/favicon.svg配置（ゴミ箱アイコン、廃棄マネージャーを表現）
        * index.htmlにfaviconリンク追加
    * **理由**
        * マスタデータ（任務・装備）にダミーデータが含まれており、正確性を保証できないため
        * ユーザーに自身で装備・任務を追加することを推奨
...
* 追加機能実装
    ...
    * キャッシュ戦略の仕様明確化 (#46)
* エラー処理強化
    * ❌ ネットワークリトライ・タイムアウト実装 (#8 obsolete - データバンドル化により不要)
    * プライベートモード検出実装 (#9)
* バックエンド実装
